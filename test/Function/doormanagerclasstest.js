...
class Door {
    constructor(name, position, targetScene = "no scene") {
        this.name = name;
    ...
    createEntity() {
        ...
        entity.setAttribute('class', 'link clickable movableBox');
        entity.setAttribute('scale', '0.5 0.5 0.5');
        entity.setAttribute('id', this.name);
        
        entity.addEventListener('click', (event) => {
            TakeDoor(event);
        });

        document.querySelector('#rightController').addEventListener('gripdown', (event) => {
    ...
    static addToScene(scene, position) {
        const doorCount = scene.tags.filter(tag => tag.type === 'door').length;
        const doorName = `door${doorCount + 1}`;
        
        const newDoor = new Door(doorName, position);
        scene.tags.push({
            type: 'door',
            position: position,
        ...
        const newEntity = newDoor.createEntity();
...
export function addDoor() {
    ...
    // Utilisation de la classe Door pour ajouter une porte à la scène
    Door.addToScene(selectedScene, position);
...
export function LoadDoors() {
    ...
    selectedScene.tags.forEach(tag => {
        if (tag.type === 'door') {
            const door = new Door(tag.name, tag.position, tag.targetScene);
            const newEntity = door.createEntity();
...
export function DuplicateDoor() {
    ...
    const originalDoor = selectedScene.tags.find(tag => tag instanceof Door && tag.name === doorName);

    if (originalDoor) {
        const newDoor = originalDoor.clone(); // En supposant que ta classe Door a une méthode clone()
        newDoor.name = `${doorName}_copy`;
        newDoor.position.x += 1; // Décalage en x de la copie

        selectedScene.tags.push(newDoor);
    }
}
export function deleteDoor() {
    ...
    const door = selectedScene.tags.find(tag => tag instanceof Door && tag.name === doorName);
